...
public class AudioPlayer : MonoBehaviour
{
    ...
    [SerializeField] AudioClip missileWarningClip;
    [SerializeField][Range(0f, 1f)] float missileWarningVolume = 1f;
    [Header("Damage")]
    ...
    public void PlayMissileWarningClip(float duration)
    {
        PlayClipWithDuration(missileWarningClip, missileWarningVolume, duration);
    }
    ...
    private void PlayClipWithDuration(AudioClip clip, float volume, float duration)
    {
        if (clip == null) return;
        GameObject tempGO = new GameObject("TempAudio");
        tempGO.transform.position = Camera.main.transform.position;
        ...
        audioSource.clip = clip;
        audioSource.volume = volume * sfxSliderValue;
        var timed = tempGO.AddComponent<TimedAudio>();
        timed.src = audioSource;
        timed.PlayFor(duration);
    }
    private IEnumerator PlayForSeconds(float duration, AudioSource audio)
    {
        audio.Play();

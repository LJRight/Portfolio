...
public class PlayerShooterManager : MonoBehaviour
{
    ...
    {
        float newRate = playerFiringRate;
        if (ratio != 0)
        {
            if (playerFiringRate != maxFireRate)
            {
                newRate = playerFiringRate * (1f - ratio / 100f);
                if (newRate < maxFireRate)
                    newRate = maxFireRate;
            }
        }
        playerFiringRate = newRate;
        foreach (Shooter shooter in shooters)
    ...
    public void ActiveFlowerShooter(int idx)
    {
        ...
        shooters[idx].gameObject.GetComponent<FollowerShooterFadeIn>().StartFadeIn();
